...
const MacroCalculator: React.FC<MacroCalculatorProps> = ({ content, lang, navigateTo, unitSystem = 'metric' }) => {
  const isAr = lang === 'ar';
  ...
  if (unitSystem !== prevUnitSystem) {
    ...
    if (baseWeight > 0) {
      setWeight(convertValue(baseWeight, 'weight', unitSystem).toFixed(1));
    }
    if (baseHeight > 0) {
      setHeight(convertValue(baseHeight, 'height', unitSystem).toFixed(1));
    }
  }
