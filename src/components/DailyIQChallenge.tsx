                ...
                <div className="max-w-2xl mx-auto bg-zinc-800/50 backdrop-blur-sm rounded-2xl p-8 border border-zinc-700">
                    {!answered ? (
                        <>
                            ...
                            <div className="grid gap-4">{question.options.map((opt, idx) => (<button key={idx} onClick={() => handleOption(idx)} className="p-4 rounded-xl bg-zinc-900 border border-zinc-700 hover:border-gold-500 hover:bg-zinc-800 transition-all font-medium text-left rtl:text-right">{opt}</button>))}</div>
                        </>
                    ) : (
                        <div className="animate-fade-in">
                            {correct ? (
                                <>
                                    ...
                                    <div className="bg-zinc-900 p-4 rounded-xl border border-zinc-700 flex items-center justify-between mb-6"><code className="text-gold-500 font-mono font-bold tracking-widest">STEROIDMASTERY25</code><button aria-label="Copy discount code" title="Copy discount code" onClick={() => { navigator.clipboard.writeText('STEROIDMASTERY25'); toast.success(content.dailyIQ.copySuccess); }} className="p-2 hover:bg-zinc-800 rounded-lg transition-colors"><Copy className="w-5 h-5 text-zinc-500 hover:text-white" /></button></div>
                                </>
                            ) : (
                                <>
