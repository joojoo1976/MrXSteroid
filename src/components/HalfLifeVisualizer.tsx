...
const HalfLifeVisualizer: React.FC<HalfLifeVisualizerProps> = ({ content }) => {
  const [stack, setStack] = useState<Array<{id: string, compoundId: string, dosage: number, frequency: string, duration: number, color: string}>>([]);
  ...
  const addToStack = () => {
    setStack([...stack, { id: Math.random().toString(36).substr(2, 9), compoundId, dosage, frequency, duration, color: colors[stack.length % colors.length] }]);
  };
    ...
    <div className="max-w-5xl mx-auto py-10">
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        <div className="bg-white dark:bg-zinc-900 p-6 rounded-3xl border border-zinc-200 dark:border-zinc-800 h-fit space-y-6">
           <div><label className="text-xs font-bold text-zinc-500 uppercase block mb-2">{content.halfLifeVisualizer.compoundLabel}</label><select value={compoundId} onChange={e => setCompoundId(e.target.value)} className="w-full bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-xl p-3 outline-none">{content.halfLifeVisualizer.compounds.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}</select></div>
           <div className="grid grid-cols-2 gap-4"><div><label className="text-xs font-bold text-zinc-500 uppercase block mb-2">{content.halfLifeVisualizer.dosageLabel}</label><input type="number" value={dosage} onChange={e => setDosage(Number(e.target.value))} className="w-full bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-xl p-3 outline-none" /></div><div><label className="text-xs font-bold text-zinc-500 uppercase block mb-2">{content.halfLifeVisualizer.durationLabel}</label><input type="number" value={duration} onChange={e => setDuration(Number(e.target.value))} className="w-full bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-xl p-3 outline-none" /></div></div>
           <div><label className="text-xs font-bold text-zinc-500 uppercase block mb-2">{content.halfLifeVisualizer.frequencyLabel}</label><select value={frequency} onChange={e => setFrequency(e.target.value)} className="w-full bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-xl p-3 outline-none">{Object.entries(content.halfLifeVisualizer.frequencies).map(([k, v]) => <option key={k} value={k}>{v as string}</option>)}</select></div>
           <button onClick={addToStack} className="w-full py-3 bg-zinc-900 dark:bg-white text-white dark:text-black font-bold rounded-xl flex items-center justify-center gap-2 hover:opacity-90 transition-opacity"><Plus className="w-5 h-5" /> Add to Stack</button>
           {stack.length > 0 && (<div className="pt-6 border-t border-zinc-200 dark:border-zinc-800 space-y-3"><h4 className="text-sm font-bold text-zinc-500 uppercase">Active Compounds</h4>{stack.map((item, idx) => { const name = content.halfLifeVisualizer.compounds.find(c => c.id === item.compoundId)?.name; return ( <div key={item.id} className="flex justify-between items-center bg-zinc-50 dark:bg-zinc-950 p-3 rounded-xl border border-zinc-200 dark:border-zinc-800 text-sm"><div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full" style={{ background: item.color }}></div><div><div className="font-bold">{name}</div><div className="text-xs text-zinc-500">{item.dosage}mg / {item.frequency} / {item.duration}wks</div></div></div><button onClick={() => removeFromStack(item.id)} className="text-zinc-400 hover:text-red-500"><Trash2 className="w-4 h-4" /></button></div> )})}</div>)}
        </div>
        <div className="lg:col-span-2 bg-white dark:bg-zinc-900 p-6 rounded-3xl border border-zinc-200 dark:border-zinc-800 relative min-h-[400px]">
           {simulationData ? (
             <div className="w-full h-full min-h-[350px] flex flex-col">
                 <div className="flex justify-between items-center mb-4"><h3 className="font-bold flex items-center gap-2"><LineChart className="w-5 h-5 text-gold-500" /> Serum Concentration</h3><div className="text-xs font-mono bg-zinc-100 dark:bg-zinc-800 px-2 py-1 rounded">Peak: {Math.round(simulationData.maxLevel)} mg equivalent</div></div>
                 <div className="flex-1 w-full relative">
                    <svg viewBox={`0 0 ${simulationData.daysToSimulate} 100`} className="w-full h-full overflow-visible" preserveAspectRatio="none">
                       ...
                       <text x={simulationData.pctStartDay + 2} y="10" fontSize="4" fill="#22c55e" fontWeight="bold">PCT START</text>
                 ...
                 <div className="flex justify-between text-xs text-zinc-400 mt-2 font-mono"><span>Day 0</span><span>Day {Math.round(simulationData.daysToSimulate / 2)}</span><span>Day {simulationData.daysToSimulate}</span></div>
             </div>
           ) : (
             <div className="h-full flex flex-col items-center justify-center text-zinc-400"><Activity className="w-16 h-16 mb-4 opacity-20" /><p>Add compounds to visualize the cycle</p></div>
           )}
        </div>
      </div>
    </div>
