...
const GeneticPotentialCalculator: React.FC<GeneticPotentialCalculatorProps> = ({ content, unitSystem = 'metric' }) => {
  ...
  const calculatePotential = () => {
    ...
    let w = parseFloat(formData.wrist);
    let a = parseFloat(formData.ankle);
    const bf = parseFloat(formData.bodyFat) / 100;
    ...
    const base = Math.pow(h, 1.5);
    const bone = (Math.sqrt(w) / 22.6670) + (Math.sqrt(a) / 17.0104);
    const leanMassLbs = base * bone;
    const totalWeightLbs = leanMassLbs / (1 - bf);

    // Natural Limit
    const naturalWeight = isImperial ? totalWeightLbs : totalWeightLbs * 0.453592;
