...
Phase 3: Schema & Migrations (+ K-2: C7 de-dupe composite key) (DELIVERED 2026-09-17 — migrations `phase3_merchant_config_tables_and_seeds`, `phase3_webhook_events_c7_dedup_key`, `phase3_webhook_events_missing_columns_and_attempt_bump`; NEW tables `product_prices`, `merchant_configs`, `kashier_product_mappings`; `products` extended with name_ar/name_en/description_ar/description_en/active; canonical catalog seeded (MRX-PROTOCOL 499 EGP, MRX-TACTICAL 749 EGP, MRX-SMART-PRO 10,848 EGP) + Egypt `merchant_configs` MID-48761-625; `webhook_events` gained `transaction_id`/`processing_status`/`raw_payload`/`updated_at` + composite UNIQUE `(provider, transaction_id, status)` (C7); bogus `rpc('coalesce')` dedup replaced by `bump_webhook_attempt` RPC; full audit 744/744)
