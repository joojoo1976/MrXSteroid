...
export function AffiliateOverview({ token }: Props) {
    ...
    if (error) {
        return (
            <div className="text-center py-12">
                ...
                <button onClick={refetch} className="text-yellow-400 underline text-sm mt-2">
                    Retry
                </button>
            </div>
        );
    ...
    if (!enrolled || !affiliate) {
        return <AffiliateOnboarding onEnroll={enroll} enrolling={enrolling} />;
    }

    const sc = affiliate.status === "active" ? "bg-green-900 text-green-400" : "bg-zinc-700 text-gray-400";
    const tabLabel = `Referrals (${totalReferrals})`;
        ...
        <div className="space-y-6 p-6 max-w-4xl mx-auto">
            ...
            <div className="space-y-4">
                ...
                {activeTab === "referrals" ? (
                    ...
                    <CommissionHistory referrals={referrals} ledger={ledger} />
                )}
            </div>
        </div>
    );
