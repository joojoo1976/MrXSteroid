...
import {
    Zap, BicepsFlexed, Trophy, Flag, Star, Droplet, Flame, Brain, Info,
    ...
    return (
        <div className="relative w-full max-w-[240px] mx-auto aspect-square" role="img" aria-label={`${leanLabel}: ${muscleStr} (${Math.round(muscle)}%) · ${fatLabel}: ${fatStr} (${Math.round(fat)}%)`}>
            ...
            <svg viewBox="0 0 200 200" className="w-full h-full drop-shadow-[0_0_18px_rgba(234,179,8,0.25)]">
                ...
                <g
                    className="cursor-pointer transition-all duration-300 hover:opacity-80 group/ring"
                ...
                <g
                    className="cursor-pointer transition-all duration-300 hover:opacity-90 group/core"
                    style={{ transformOrigin: '100px 100px' }}
                    role="button"
                    tabIndex={0}
            ...
            <AnimatePresence>
                {activeTooltip && (
                    ...
                    >
                        <div className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-zinc-900/95 border border-gold-500/40 text-white shadow-2xl backdrop-blur-xl whitespace-nowrap">
...
/** Detailed Explanation Dialog Box for "Muscle x Fat" Graphic */
const KineticGraphicExplanationBox: React.FC<{
    isAr: boolean;
    quality: BodyQuality;
    muscleKg: number;
    fatKg: number;
    unitSystem: UnitSystem;
}> = memo(({ isAr, quality, muscleKg, fatKg, unitSystem }) => {
    const [open, setOpen] = useState(true);
    const muscleStr = formatWeight(muscleKg, unitSystem, isAr);
    const fatStr = formatWeight(fatKg, unitSystem, isAr);

    return (
        <div className="w-full mt-3 rounded-2xl bg-zinc-900/90 border border-gold-500/30 p-3.5 text-[11px] leading-relaxed backdrop-blur-xl shadow-xl">
            <div className="flex items-center justify-between gap-2 border-b border-zinc-800 pb-2 mb-2">
                <span className="flex items-center gap-2 text-gold-400 font-black uppercase tracking-wider text-[11px]">
                    <Info className="w-4 h-4 text-gold-400 shrink-0 animate-pulse" />
                    {isAr ? '💡 الشرح التفصيلي لرسمة كتلة عضلية × نسبة دهون' : '💡 Detailed Guide: Muscle x Fat Graphic'}
                </span>
                <button
                    type="button"
                    onClick={() => setOpen((prev) => !prev)}
                    className="text-[10px] font-black px-2.5 py-1 rounded-lg bg-gold-500/10 border border-gold-500/30 text-gold-400 hover:bg-gold-500/20 transition-all"
                >
                    {open ? (isAr ? 'إخفاء الشرح' : 'Hide Details') : (isAr ? 'عرض الشرح التفصيلي' : 'Show Guide')}
                </button>
            </div>
            ...
            <AnimatePresence>
                {open && (
                    <motion.div
                        ...
                        className="space-y-2 text-zinc-300 font-medium overflow-hidden"
                    >
                        <p className="text-zinc-300 text-[11px] font-bold">
                            {isAr
                                ? 'تتفاعل الرسمة الحيوية وتتغير أبعادها ومكوناتها لحظياً مع أي تغيير في السلايدرز:'
                                : 'This graphic adapts live as you adjust the input sliders:'}
                        </p>
                            ...
                            <div className="p-2.5 rounded-xl bg-blue-500/10 border border-blue-500/20 flex items-start gap-2">
                                ...
                                <div className="leading-relaxed">
                                    <span className="font-black text-blue-400 uppercase">
                                        {isAr ? 'الحلقة الخارجية (كتلة الدهون):' : 'Outer Blue Ring (Fat Mass):'}
                                    </span>{' '}
                                    {isAr
                                        ? `تزن ${fatStr}. يتغير قطرها وسُمكها طردياً مع نسبة الدهون؛ ينكمش هذا الغلاف مع تقدم حرق الدهون وظهور البروز العضلي.`
                                        : `Carries ${fatStr}. Thickness shrinks as body fat drops.`}
                                </div>
                            </div>

                            <div className="p-2.5 rounded-xl bg-gold-500/10 border border-gold-500/20 flex items-start gap-2">
                                <span className="w-2.5 h-2.5 rounded-full bg-gold-400 shrink-0 mt-0.5" />
                                <div className="leading-relaxed">
                                    <span className="font-black text-gold-400 uppercase">
                                        {isAr ? 'النواة الداخلية (الكتلة الصافية):' : 'Inner Core (Lean Muscle Mass):'}
                                    </span>{' '}
                                    {isAr
                                        ? `تزن ${muscleStr}. تُعبر عن الأنسجة العضلية والهيكل العظمي. تتوسع وتنمو بحجم أكبر مع زيادة البناء العضلي التراكمي.`
                                        : `Carries ${muscleStr}. Expands with cumulative muscle gain.`}
                                </div>
                            </div>

                            <div className="p-2.5 rounded-xl bg-purple-500/10 border border-purple-500/20 flex items-start gap-2">
                                <span className="w-2.5 h-2.5 rounded-full bg-purple-400 shrink-0 mt-0.5" />
                                <div className="leading-relaxed">
                                    <span className="font-black text-purple-400 uppercase">
                                        {isAr ? 'نسبة التناسب ومؤشر الجودة:' : 'Muscle : Fat Ratio & Quality Score:'}
                                    </span>{' '}
                                    {isAr
                                        ? `تصل نسبة العضلات إلى الدهون حالياً إلى ${quality.muscleFatRatio}:1 بمؤشر جودة قوام يبلغ ${quality.qualityScore}/100.`
                                        : `Muscle to fat ratio is ${quality.muscleFatRatio}:1 with quality index ${quality.qualityScore}/100.`}
                                </div>
                            </div>
                        </div>
                    </motion.div>
