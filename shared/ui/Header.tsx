      ...
      case 'lang-theme': {
        ...
        return (
          <div key="lang-theme" className="flex items-center gap-2">
            ...
            {/* Direct Language Switcher (AR | EN) with Explicit Route Sync */}
            <button
              onClick={() => {
              ...
              className="group flex items-center justify-center gap-1.5 px-2.5 py-1.5 h-9 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white/80 dark:bg-zinc-900/80 hover:bg-zinc-100 dark:hover:bg-zinc-800/90 hover:border-zinc-300 dark:hover:border-zinc-700 transition-all text-xs font-black text-zinc-800 dark:text-zinc-100 shadow-xs outline-none select-none cursor-pointer"
              title={isRTL ? 'التغيير إلى English (/en)' : 'Switch to العربية (/ar)'}
              aria-label={isRTL ? 'تغيير اللغة' : 'Switch Language'}
            >
              <span className="font-mono text-[11px] font-black uppercase text-gold-500 group-hover:scale-105 transition-transform">
                {lang === Language.AR ? 'EN' : 'عربي'}
              </span>
            </button>

            {/* Contextual Unit Switcher Button - Shown ONLY on interactive calculator pages */}
            {isCalculatorPage && (
              <button
          ...
          <div key="nav" className="hidden md:flex items-center gap-4">
            ...
            <div className="relative group">
              ...
              <div className={`absolute top-full ${isRTL ? 'end-0' : 'start-0'} pt-2 w-64 hidden group-hover:block animate-fade-in-up z-50`}>
                <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl shadow-xl overflow-hidden">
                  <button onClick={() => navigateTo(Page.TIMELINE)} className="block w-full text-start px-3 py-2 text-xs hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors flex items-center gap-2">
                    <span className="text-gold-500">🏆</span>{isRTL ? 'الجدول الزمني للتحول الجسدي' : 'Transformation Timeline'}
                  </button>
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          ...
          >
            ...
            <button onClick={() => { navigateTo(Page.TIMELINE); setIsMobileMenuOpen(false); }} className="text-sm text-start ps-4 flex items-center gap-2">
              <span className="text-gold-500">🏆</span>{isRTL ? 'الجدول الزمني للتحول الجسدي' : 'Transformation Timeline'}
            </button>
